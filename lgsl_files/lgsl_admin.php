...
//------------------------------------------------------------------------------------------------------------+
  ...
  if (!empty($_POST['lgsl_save_1']) || !empty($_POST['lgsl_save_2']))
  {
    if (!empty($_POST['lgsl_save_1']))
    {
      ...
      $db = array();
      $query = "SELECT * FROM {$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}";
      $result = mysqli_query($link, $query);
      
      while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
      {
        $db["{$row['type']}:{$row['ip']}:{$row['q_port']}"] = array($row['status'], $row['cache'], $row['cache_time']);
      }
    }

    // EMPTY SQL TABLE
    $query = "TRUNCATE `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}`";
    ...
    foreach ($_POST['form_type'] as $form_key => $not_used)
    {
      ...
      $type       = mysqli_real_escape_string($link, strtolower(trim($_POST['form_type'][$form_key])));
      $ip         = mysqli_real_escape_string($link, trim($_POST['form_ip']             [$form_key]));
      $c_port     = mysqli_real_escape_string($link, intval(trim($_POST['form_c_port']  [$form_key])));
      $q_port     = mysqli_real_escape_string($link, intval(trim($_POST['form_q_port']  [$form_key])));
      $s_port     = mysqli_real_escape_string($link, intval(trim($_POST['form_s_port']  [$form_key])));
      $zone       = mysqli_real_escape_string($link, trim($_POST['form_zone']           [$form_key]));
      $disabled   = isset($_POST['form_disabled'][$form_key]) ? intval(trim($_POST['form_disabled'][$form_key])) : "0";
      $comment    = mysqli_real_escape_string($link, trim($_POST['form_comment']        [$form_key]));
      ...
      $cache      = mysqli_real_escape_string($link, $cache);
      $cache_time = mysqli_real_escape_string($link, $cache_time);
...
//---------------------------------------------------------+
        ...
        $query = "SELECT * FROM `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` ORDER BY `id` ASC";
        $result = mysqli_query($link, $query);
