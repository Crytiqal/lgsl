...
//-----------------------------------------------------------------------------------------------------------+
  ...
  $q_port = mysqli_real_escape_string($link, $q_port);
  $c_port = mysqli_real_escape_string($link, $c_port);
  $s_port = mysqli_real_escape_string($link, $s_port);
  $type   = mysqli_real_escape_string($link, $type);

//-----------------------------------------------------------------------------------------------------------+

  $ip_check  = gethostbyname($ip);
  $query = "SELECT `ip`,`disabled` FROM `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` WHERE `type`='{$type}' AND `q_port`='{$q_port}'";
  $result = mysqli_query($link, $query);

  while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
  {
    if ($ip_check == gethostbyname($row['ip']))
    {
      $output .= "
      <div style='text-align:center;".lgsl_bg()."'>
        <br />";
...
//-----------------------------------------------------------------------------------------------------------+
  ...
  if (!empty($_POST['lgsl_submit_add']))
  {
    ...
    $query  = "INSERT INTO `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` (`type`,`ip`,`c_port`,`q_port`,`s_port`,`disabled`,`cache`,`cache_time`) VALUES ('{$type}','{$ip}','{$c_port}','{$q_port}','{$s_port}','{$disabled}','','')";
    $result = $mysqli_query($link, $query);
