...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_link($s = "")
  {
    ...
    {
      case "e107":
        $url = $s ? e_PLUGIN."lgsl/{$index}?s={$s}" : e_PLUGIN."lgsl/{$index}";
      break;

      case "joomla":
        $url = $s ? JRoute::_("index.php?option=com_lgsl&s={$s}") : JRoute::_("index.php?option=com_lgsl");
      break;

      case "drupal":
        $url = $s ? url("LGSL/{$s}") : url("LGSL");
      break;

      case "phpnuke":
        $url = $s ? "modules.php?name=LGSL&s={$s}" : "modules.php?name=LGSL";
      break;

      default: // "sa"
        $url = $s ? $lgsl_url_path."../{$index}?s={$s}" : $lgsl_url_path."../{$index}";
      break;
    }
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_database()
  {
    ...
    // $lgsl_select_db = mysqli_select_db($lgsl_database, $lgsl_config['db']['db']) or die(mysqli_error());
    // $mysqli = new mysqli($lgsl_config['db']['server'], $lgsl_config['db']['user'], $lgsl_config['db']['pass'], $lgsl['db']['db']);
    $link = mysqli_connect($lgsl_config['db']['server'], $lgsl_config['db']['user'], $lgsl_config['db']['pass'], $lgsl_config['db']['db']);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_query_cached($type, $ip, $c_port, $q_port, $s_port, $request, $id = NULL)
  {
    ...
    {
      $id     = intval($id);
      $query  = "SELECT * FROM `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` WHERE `id`='{$id}' LIMIT 1";
    ...
    {
      list($type, $ip, $c_port, $q_port, $s_port) = array(mysqli_real_escape_string($link, $type), mysqli_real_escape_string($link, $ip), intval($c_port), intval($q_port), intval($s_port));

      if (!$type || !$ip || !$c_port || !$q_port) { exit("LGSL PROBLEM: INVALID SERVER '{$type} : {$ip} : {$c_port} : {$q_port} : {$s_port}'"); }
      $query  = "SELECT * FROM `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` WHERE `type`='{$type}' AND `ip`='{$ip}' AND `q_port`='{$q_port}' LIMIT 1";
      $result = mysqli_query($link, $query);
      $row    = mysqli_fetch_array($result, MYSQLI_ASSOC);
      mysqli_free_result($result);
      if (!$row)
      {
        ...
        $query  = "INSERT INTO `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` (`type`,`ip`,`c_port`,`q_port`,`s_port`,`cache`,`cache_time`) VALUES ('{$type}','{$ip}','{$c_port}','{$q_port}','{$s_port}','','')";
        $result = mysqli_query($link, $query);
        $row    = array("id"=>mysqli_insert_id(), "zone"=>"0", "comment"=>"");
        mysqli_free_result($result);
      }
    }
    ...
    if ($needed)
    {
      ...
      $query  = "UPDATE `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` SET `cache_time`='{$packed_times}' WHERE `id`='{$row['id']}' LIMIT 1";
      ...
      $packed_cache = mysqli_real_escape_string($link, base64_encode(serialize($cache)));
      $packed_times = mysqli_real_escape_string($link, implode("_", $cache_time));
      $query  = "UPDATE `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` SET `status`='{$cache['b']['status']}',`cache`='{$packed_cache}',`cache_time`='{$packed_times}' WHERE `id`='{$row['id']}' LIMIT 1";
      mysqli_query($link, $query);
...
//EXAMPLE USAGE: lgsl_query_group( array("request"=>"sep", "hide_offline"=>0, "random"=>0, "type"=>"source", "game"=>"cstrike") )
  ...
  function lgsl_query_group($options = array())
  {
    ...
    $query  = "SELECT * FROM `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` WHERE (`zone`=0 OR `zone`='{$zone}') AND (`disabled`=0) ORDER BY {$mysql_order}";
    $result = mysqli_query($link, $query);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_lookup_id($id) // LEGACY - DO NOT USE
  {
    ...
    $id     = mysqli_real_escape_string($link, intval($id));
    $query  = "SELECT `type`,`ip`,`c_port`,`q_port`,`s_port` FROM `{$lgsl_config['db']['prefix']}{$lgsl_config['db']['table']}` WHERE `id`='{$id}' LIMIT 1";
    $result = mysqli_query($link, $query);
    $row    = mysqli_fetch_array($result, MYSQLI_ASSOC);
    mysqli_free_result($result);
